...
def _get_additional_info(input_type: str) -> str:
    ...
    if input_type not in ["node", "relationship", "property"]:
        raise ValueError(
            "input_type must be 'node', 'relationship', or 'property'")
...
def create_unstructured_prompt(
    ...
    base_string_parts = [
        ...
        "Attempt to extract as many entities and relations as you can. Do not repeat unnecesary relations. Maintain "
    ...
    def create_openai_messages(input_text: str) -> List[Dict[str, str]]:
        return [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": human_prompt.format(input=input_text)}
        ]

    return chat_prompt, create_openai_messages
...
def create_simple_model(
    ...
    if node_properties:
        if isinstance(node_properties, list) and "id" in node_properties:
            raise ValueError(
                "The node property 'id' is reserved and cannot be used.")
    ...
    SimpleRelationship = create_model(
        "SimpleRelationship", **relationship_fields)  # type: ignore

    class DynamicGraph(_Graph):
        """Represents a graph document consisting of nodes and relationships."""

        nodes: Optional[List[SimpleNode]] = Field(
            description="List of nodes")  # type: ignore
